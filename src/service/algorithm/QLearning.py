...
class QLearning:
    ...
    def step(self, action):
        ...
        # FIXME: rewrite this
        if 60 > self.current_pos[0] + self.possibleDirections[action][0] >= 0 and 60 > self.current_pos[1] + \
                self.possibleDirections[action][1] >= 0:
            ...
            for i in range(self.max_iter_episode):
                ...
                if np.random.uniform(0, 1) < self.exploration_proba:
                    action = random.choice(
                        range(len(self.possibleDirections)))  # env.action_space.sample() #choose random action
                else:
                ...
                QTable[current_state, action] = (1 - self.lr) * QTable[current_state, action] + self.lr * (
                            reward + self.gamma * max(QTable[next_state, :]))
